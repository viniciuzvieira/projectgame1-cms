...
class CardDeckServiceTest extends TestCase
{
    ...
    public function test_creation_and_quantity_updates_are_idempotent(): void
    {
        $deck = $this->service->create($this->userId, '  Forja  ');
        $again = $this->service->create($this->userId, 'Forja');
        $this->assertSame($deck->id, $again->id);
        $this->assertSame('Forja', $deck->name);
        $this->service->setCard($this->userId, $deck->id, $this->cardId, 2);
        $this->service->setCard($this->userId, $deck->id, $this->cardId, 2);
        $this->assertSame(1, DB::table('user_card_deck_cards')->count());
        $this->assertSame(2, $this->service->listing($this->userId)[0]['cards'][0]['quantity']);
    ...
    public function test_removing_a_card_or_deleting_a_deck_preserves_collection(): void
    {
    ...
    public function test_unowned_cards_and_excess_copies_are_rejected(): void
    {
        ...
        foreach ([[$this->cardId, 3], [$this->cardId, -1], [$unowned, 1]] as [$id, $quantity]) {
            try {
    ...
    public function test_every_mutation_is_scoped_to_its_owner(): void
    {
        ...
        $this->assertSame('Privado', $deck->fresh()->name);
    }
    ...
    {
        foreach (['store', 'rename', 'card', 'primary', 'destroy'] as $action) {
