...
export const ChatInputView: FC<ChatInputViewProps> = props => {
    const {
        usePortal = true,
        portalTargetId = 'toolbar-chat-input-container',
        closeOnEnter = false,
        onRequestClose = null
    } = props;
    ...
    const onKeyDownEvent = useCallback((event: KeyboardEvent) => {
        ...
        const isChatInputTarget = (event.target === inputRef.current);
        const value = (isChatInputTarget ? (event.target as HTMLInputElement).value : inputRef.current.value);
        ...
        switch (event.key) {
            ...
            case 'Enter':
                ...
                // fecha só se o Enter foi no input do chat
                if (closeOnEnter && onRequestClose && isChatInputTarget) {
    ...
    }, [floodBlocked, inputRef, chatModeIdWhisper, anotherInputHasFocus, setInputFocus, checkSpecialKeywordForInput, sendChatValue, closeOnEnter, onRequestClose]);
    ...
    useEffect(() => {
        (window as any).__kbSendChat = (text: string, shout: boolean = false) => {
            // shout true = manda como shiftKey (grito)
            sendChatValue(text, shout);
        };

        (window as any).__kbFocusChat = () => {
            setInputFocus();
        };

        return () => {
            try {
                delete (window as any).__kbSendChat;
                delete (window as any).__kbFocusChat;
            }
            catch { }
        };
    }, [sendChatValue, setInputFocus]);
