(() => {
  ...
  function getNitroChatInput(iframeId){
    ...
    try{
      ...
      return doc.querySelector('#toolbar-chat-input-container input.chat-input')
  ...
  function handleKeydown(e){
    ...
    if (e.key === 'Enter'){
      ...
      // dentro do case 'Enter' do handleKeydown
  ...
  function enableDrag(){
    ...
    frame.addEventListener('pointerup', () => {
      ...
      try{
        const rect = frame.getBoundingClientRect();
        localStorage.setItem(state.opts.storageKey, JSON.stringify({ left: rect.left, top: rect.top }));
      }catch{}
    });
  }
